...
def assemble_rmsd_matrix2 (CSVLeftPath,CSVCenterLeftPath,CSVCenterRightPath,CSVDownPath,CSVFullPath):
    ...
    ListCenterLeft = from_csv(CSVCenterLeftPath)
    ListCenterRight = from_csv(CSVCenterRightPath)    
    print len(ListCenterRight[1])
    print len(ListCenterRight[-1])#; quit()

#    print ListCenter

    for N1 in range(484):
        
        for N2 in range(484,484+242):
            print N1, N2
            RMSDMatrixI.itemset((N1,N2),(ListCenterLeft[N1][N2-484]))
            RMSDMatrixI.itemset((N2,N1),(ListCenterLeft[N1][N2-484])) 

        for N2 in range(484+242,length):
            print N1, N2
            print N2-484-242
            print ListCenterRight[N1][N2-484-242-1]
            RMSDMatrixI.itemset((N1,N2),(ListCenterRight[N1][N2-484-242]))
            RMSDMatrixI.itemset((N2,N1),(ListCenterRight[N1][N2-484-242])) 
    ...
    for N1 in range(484):
        ...
        for N2 in range(N1+1,483):
            print N1, N2-N1-1
            RMSDMatrixI.itemset((N1,N2),(ListLeft[N1][N2-N1-1]))
            RMSDMatrixI.itemset((N2,N1),(ListLeft[N1][N2-N1-1]))     
    
    ListDown = from_csv(CSVDownPath)


    for N1 in range(483+1,length):
        
        for N2 in range(483+1+N1,484):
        
            RMSDMatrixI.itemset((N1,N2),(ListDown[N1-484][N2-N1-484-1]))
            RMSDMatrixI.itemset((N2,N1),(ListDown[N1-484][N2-N1-484-1]))     
    
    to_csv(RMSDMatrixI,CSVFullPath)
# nie wiem czy dobrze, musze sie z tym polozyc, to bedzie liczone tez na tym drugim kompie    
#    return RMSD_MatrixI
...
#            print RMSDI#;quit()
        ...
        if N1 % 100 == 0:
            ...
            os.system('cp '+f_rms_path+' '+f_rms_path+'_backup_'+str(N1))
            os.system('cp '+f_rmsd_path+' '+f_rmsd_path+'_backup_'+str(N1))
...
def dir_pairwise_cross_align_first_half(dir1, dir2, fout):
    ...
    f=open('files_cross_first_half.txt','w')
...
#    RMSMatrixI = np.zeros((length1,length1))
    ...
    for N1 in range(length1):
        ...
        for N2 in range(int(float(length2)/2.0)):
...
def dir_pairwise_cross_align_second_half(dir1, dir2, fout):
    ...
    f=open('files_cross_second_half.txt','w')
...
#    RMSMatrixI = np.zeros((length1,length1))
    ...
    for N1 in range(length1):
        ...
        for N2 in range(int(float(length2)/2.0),length2):
            print 'doing stuff'
            print '   '+str(N2)
...
#assemble_rmsd_matrix (CSVLeftPath,CSVDownPath,CSVCenterPath,CSVFullPath)
#dir_pairwise_salign(sys.argv[1], sys.argv[2])
